...
chance='30/((#*(1/(@/2)))*(@/2))', -- Eu fiz essa equação para variar de +0 a +7 o item 
...
local asdeflol = {17, 18, 19, 20} -- vocações que refinam 
...
return doPlayerSendTextMessage(cid, 24,"O seu item ja está no maximo.") 
else 
...
doSendMagicEffect(toPosition, 2) 
return doPlayerSendTextMessage(cid, 24,"O item está no nível máximo, você não pode atualizá-lo.") 
elseif level > 0 then 
if level >= 1 then 
doTransformItem(itemEx.uid, 2256) 
doPlayerSendTextMessage(cid, 20,"Você falhou.") 
